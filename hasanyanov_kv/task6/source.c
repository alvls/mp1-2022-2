...
void directive(char* path, int sort, int variant_sort) // Введем функцию, которая будет заполнять массив характеристиками файлов, сортировать его и выводить на экран.
{
    ...
    else
    {
        ...
        //printf("%20c Список файлов (%d):\n\n", ' ', count);
        printf("\nФайл:                        Дата: %13c  Размер:\n\n", ' ');
        _findclose(hFile);
...
void main()
{
    ...
    int mode1, mode2;
    char path[200 + 1];
    char menu[SIZE_MENU][30] = { "BubbleSort", "SelectSort", "InsertSort", "MergeSort", "QuickSort", "ShellSort", "CountingSort", "Choose new ditectory", "Exit" };
