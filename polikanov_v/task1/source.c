...
void main() {
	...
	if (h < 209) {
		sumOfShelfs = 3; /* потому что свободного места в зависимости от высоты шкафа от 177 до 217 см.Каждая полка должна быть на расстоянии 40 см
						от другой. с учетом их толщины равной 15 мм(дсп) и того, что вверху самой последней полки должно оставаться 40см
						(так как маленькая низкая полка никому не нужна), получаем, что если шкаф ниже 215 см, мы ставим 3 полки. Выше - 4 полки.
						Если нужна будет формула, можно составить. Но исходя из логики и данных, можно не производить вычислений.
						формула sumOf = (int)((h - 3 - 40) / 41.5)
						*/
	}
	else {
		sumOfShelfs = 4;
