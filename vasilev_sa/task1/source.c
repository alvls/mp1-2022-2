...
void main() 
{
	...
	value_1 = (h * w * t1) * material_DVP;								// задняя стенка из ДВП
	value_2 = 2 * (h * d * t2) * material_DSP;							// боковые стенки из ДСП
	value_3 = 2 * (w * d * t3) * material_DSP;							// крышки из ДСП
	value_4 = (h * w * t4) * material_wood;								// двери из дерева
	value_5 = (w * d * t5) * count * material_DSP;						// внутренние полки из ДСП
	// весь объём
